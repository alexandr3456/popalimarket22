# 5min BTC Polymarket Research + Paper Engine

The repository now contains an independent BTC probability and Polymarket edge
engine. The new path is **paper/backtest only**: it has no wallet loading, order
placement, private-key handling, external live runner, or `--execute` option.

## New safe research path

- `feature_engine.py` — exact-window BTC reference, timing, momentum, volatility,
  public trade-pressure and BTC book features. Missing optional inputs remain null.
- `probability_engine.py` — rule-based score and optional coefficient-driven
  logistic mode. Polymarket prices are not accepted as predictor inputs.
- `edge_engine.py` — symmetric UP/DOWN effective costs, edges and all safety gates.
- `paper_trading.py` — settlement and repricing paper exits.
- `research_data_collector.py` — append every TRADE/NO TRADE snapshot to JSONL/CSV.
- `paper_strategy.py` — offline one-cycle paper CLI with `--debug-data`.
- `backtest_confidence_strategy.py` — chronological 70/30 reports, baselines,
  buckets, calibration, Brier score, log loss and risk/P&L metrics.
- `telegram_paper.py` — message formatting only; it performs no network action.

Run the offline engine with an adapter-produced JSON payload:

```bash
python paper_strategy.py --input snapshot.json --status --debug-data
python backtest_confidence_strategy.py data/research_data.jsonl
python -m unittest discover -s tests -v
```

Input BTC observations must include an observation within two seconds of the
actual Polymarket window start. A random rolling five-minute move is rejected.
See `DATA_AVAILABILITY.md` for the data contract and known limitations.

## Legacy live contour (not used by the research engine)

The files under `scripts/` predate this research engine and include delegation to
an external live runner. They are retained for history, but none of the new modules
imports or invokes them. Do not use that legacy contour for paper research.

Original OpenClaw skill for **BTC 5-minute Up/Down** markets on Polymarket.
